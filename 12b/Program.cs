...
namespace _12b
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(state.Waypoint);
    ...
    class ShipState
    {
        public ShipState((int X, int Y) position, (int DX, int DY) waypoint)
        {
            Position = position;
            Waypoint = waypoint;
        }
        ...
        public (int DX, int DY) Waypoint { get; set; }

        public static ShipState Initial => new ShipState((0, 0), (10, 1));

        public override string ToString()
        {
            return $"Pos: {Position}, Waypoint (rel): {Waypoint}";
        }
    }
    ...
    class Instruction
    {
        ...
        public ShipState Execute(ShipState state)
        {
            ...
            {
                if ((Direction == 'L' && Parameter == 90) || (Direction == 'R' && Parameter == 270))
                {
                    return new ShipState(state.Position, (-state.Waypoint.DY, state.Waypoint.DX));
                }
                if (Parameter == 180)
                {
                    return new ShipState(state.Position, (-state.Waypoint.DX, -state.Waypoint.DY));
                }
                if ((Direction == 'L' && Parameter == 270) || (Direction == 'R' && Parameter == 90))
                {
                    return new ShipState(state.Position, (state.Waypoint.DY, -state.Waypoint.DX));
                }
            }
            ...
            if (Direction == 'F')
            {
                var newX = state.Position.X + state.Waypoint.DX * Parameter;
                var newY = state.Position.Y + state.Waypoint.DY * Parameter;

                return new ShipState((newX, newY), state.Waypoint);
            }
            ...
            var newWaypointX = state.Waypoint.DX + moveDirection.DX * Parameter;
            var newWaypointY = state.Waypoint.DY + moveDirection.DY * Parameter;

            return new ShipState(state.Position, (newWaypointX, newWaypointY));
        }
