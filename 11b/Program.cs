...
namespace _11b
{
    class Program
    {
        ...
        {
            string boardString = @"LLLL.LLLLL.LLLLLLLLLLLLLLLL.L.LLLLLL.LLLL..L.LLLLLLLL.LLLLLL.LLLL..LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
...
";
            ...
            while (change)
            {
                ...
                for (int row = 0; row < board.Length; row++)
                {
                    ...
                    for (int col = 0; col < board[row].Length; col++)
                    {
                        ...
                        var visibleSeats = AllVisibleSeats(board, row, col).ToList();


                        if (oldState == 'L' && !visibleSeats.Any(n => n == '#'))
                        {
                            newState = '#';
                        }

                        if (oldState == '#' && visibleSeats.Count(n => n == '#') >= 5)
                        {
                            newState = 'L';
                        }
        ...
        public static void PrintBoard(char[][] board)
        {
            ...
            for (int row = 0; row < board.Length; row++)
            {
                Console.WriteLine(new string(board[row]));
            }
        }

        public static IEnumerable<char> AllVisibleSeats(char[][] board, int row, int col)
        {
            return VisibleSeats(board, row, col, -1, 0)
                .Concat(VisibleSeats(board, row, col, 1, 0))
                .Concat(VisibleSeats(board, row, col, 0, -1))
                .Concat(VisibleSeats(board, row, col, 0, 1))
                .Concat(VisibleSeats(board, row, col, -1, -1))
                .Concat(VisibleSeats(board, row, col, -1, 1))
                .Concat(VisibleSeats(board, row, col, 1, -1))
                .Concat(VisibleSeats(board, row, col, 1, 1));
            // for (int i = row; i >= 0; i--)
        ...
        public static IEnumerable<char> VisibleSeats(char[][] board, int row, int col, int rowStep, int colStep)
        {
            for (int i = row + rowStep, j = col + colStep;
        ...
        // public static IEnumerable<T> Neighbors<T>(T[][] board, int row, int col, int size = 1)
        // {
