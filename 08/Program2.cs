...
"jmp +1"	
		...
		var instructionToFlip = Enumerable.Range(0, parsedInstructions.Length)
             .Where(i => parsedInstructions[i].Operation == "nop" || parsedInstructions[i].Operation == "jmp")
             .ToList();
		
		foreach(var toFlip in instructionToFlip)
		{
			...
			while(currentInstruction < parsedInstructions.Length && !instructionsRun.Contains(currentInstruction))
			{
				...
				(int nextInstruction, var newAccumulator) = currentInstruction == toFlip ? 
					instruction.ExecuteFlipped(currentInstruction, accumulator)
					: instruction.Execute(currentInstruction, accumulator);
			...
			{
				Console.WriteLine("Flipped:" + toFlip.ToString());
